	...
	$.pageRuler = function(params){
		...
		var f = {
			...
			ui: function(){
				...
				for(var i=0; i<w; i+=2){
					if(i % 50 === 0){
						$('<span class="n">'+i+'</span>').css("left", i+1).appendTo(rh);
					}
				}
				//垂直标尺数值
				for(var i=0; i<h; i+=2){
					if(i % 50 === 0){
						$('<span class="n">'+i+'</span>').css("top", i+1).appendTo(rv);
					}
				}
			},
			ie6: function(){
