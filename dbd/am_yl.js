            ...
            success: function(response) {
               ...
               // console.info(ifDp);
                if(isAlive===true){
                	...
                	//判断是否为多屏
                	if(ifDp!='dp'){
                		var dbd='<iframe src="dbd2_yl.fh5?key='+data.dbdView+'&'+amCS+'" frameborder="0" width="100%" height="1000px" scrolling="auto"></iframe>';

                	}else{
                		var dbd='<iframe src="more_yl.fh5?key='+data.dbdView+'&'+amCS+'" frameborder="0" width="100%" height="1000px" scrolling="auto"></iframe>';
                	}
	...
	function targetC(domName,t,dbdk,cs){
		...
		if(t=='null'||''){
			...
			$("#two").dialog({
		  	...
		  	$("#two iframe").attr('src','dbd2_view_yl.fh5?key='+dbdk+'&'+cs);
		}else if(t=='down'){
			window.frames[domName].location.href='dbd2_view_yl.fh5?key='+dbdk+'&'+cs;
		}else if(t=='blank'){
			parent.window.open('dbd2_view_yl.fh5?key='+dbdk+'&'+cs);
		}
		/*else if(t=='link'){
			...
			$.ajax({
		        ...
		        success:function(res){
		        	//console.info(res);
		        	var d2=res[0].dtitle;
		        	window.location.href='dbd2_yl.fh5?key='+dbdk+'&'+cs+'&d2title='+d2;
		        }
			});
